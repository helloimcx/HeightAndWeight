                    ...
                    if (jsonRecordBean != null) {
                        ...
                        if (count_record <= 1) {
                            ...
                            for (int i = 0; i <= count_record - 1; i++) {
                                ...
                                String date = "";
                                if(i==0 || i==count_record - 1){
                                    date = records.get(i).get_record_time().substring(0,10);
                                }
                                else {
                                    date = records.get(i).get_record_time().substring(5,10);
                                }
                                dataList.add(new Entry((float)i,(float)weight));
                                xValues[i] = date;
                            }
                            LineDataSet dataSet = new LineDataSet(dataList, "体重"); // add entries to dataset
